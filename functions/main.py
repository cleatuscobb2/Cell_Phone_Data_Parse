...
def handle_custody(req: https_fn.Request) -> dict:
    ...
    doc_payload = [
        (df.read(), df.filename or "") for df in _uploads(req, "document_files")
    ]
    if doc_payload:
        _merge_document_evidence(report, _extract_documents(doc_payload, context))
...
def handle_jobs_create(req: https_fn.Request, uid: str) -> dict:
    ...
    if kind == "custody":
        ...
        # Supporting documents are read here too (bounded vision work) and
        # ride in the job blob to merge into the report at assembly.
        doc_payload = [
            (df.read(), df.filename or "") for df in _uploads(req, "document_files")
        ]
        docs = _extract_documents(doc_payload, context) if doc_payload else None
        ...
        blob = {
            ...
            "documents": docs.model_dump() if docs else None,
            "meta": meta,
            "transcript": messages_to_records(all_messages[:TRANSCRIPT_CAP]),
        }
...
def _assemble_job(uid: str, job_id: str, job: dict) -> dict:
    ...
    if kind == "custody":
        ...
        if blob.get("documents"):
            _merge_document_evidence(
                report, DocumentExtraction(**blob["documents"]))
        if skipped:
